    ...
    while((buffer->readIndex) != (buffer->writeIndex))
    {
        ...
        if(gotNewLength)
        {
            if((buffer->readPacketIndex + buffer->readExpLen) > LN_BUF_SIZE)
            {
                ...
                // The packet won't fit in the remaining part of the buffer without wrapping
...
{
    if((msg->sz.command & (uint8_t)0x60) == (uint8_t)0x60)
        return msg->sz.mesg_size;
    else
        return ((msg->sz.command & (uint8_t)0x60) >> (uint8_t)4) + 2;
}
